...
export class AuthController {
  ...
  readonly #client: SignexClient;

  public constructor(private readonly store: SessionStore) {
    const extra = Constants.expoConfig?.extra as Record<string, unknown> | undefined;
    const baseUrl = typeof extra?.apiBaseUrl === "string" ? extra.apiBaseUrl : "https://staging.signex.invalid";
    this.#client = new SignexClient({
      baseUrl,
      getAccessToken: () => this.#accessToken,
    });
  }

  public get client(): SignexClient {
    return this.#client;
  }
  ...
  async #acceptSession(session: SessionCredentials): Promise<void> {
    ...
    try {
      ...
      throw new Error("Signex could not protect the refresh credential on this device.", { cause: error });
    }
  }
  ...
  public async restore(): Promise<CurrentUser | null> {
    ...
    try {
      ...
      if (error instanceof SignexApiError && isSessionInvalidatingCode(error.code)) {
        await this.clear();
      }
      throw error;
    }
