...
| Field | Value |
|---|---|
| Product name | **Signex** (working public name; complete trademark, domain, app-store and social-handle clearance before commercial release) |
| Mobile display name | **Signex** |
...
**Brand direction:** **Signex** combines *signal* with *execution*. It positions the product as a real-time system that detects funding opportunities and turns approved signals into controlled, two-leg trades. The working brand line is *Signals into execution.* Speed and velocity are visual and operational themes, not promises of atomic cross-venue fills, guaranteed latency, neutrality or profit. The internal `funding-arb` slug remains independent of the public brand.
...
- Interim exception (DEC-0009): the shared v1 backend may run with `SIGNEX_OPEN_REGISTRATION` enabled, which admits each new social sign-in immediately to full access without pending approval or MFA. Version 2 isolates each user's backend and restores the gated approval and mandatory-MFA rules above.
...
The product name is written **Signex** in title case in normal copy. An uppercase `SIGNEX` treatment is permitted only in the approved wordmark. The identity should express signal detection, velocity and controlled execution without making the interface resemble a casino or implying that execution is risk-free:

- Build the primary mark around a forward-moving signal path: two measured traces or velocity vectors converge on an execution node and resolve into two coordinated hedge legs. A simplified angular `S` may emerge from the negative space, but it must remain readable rather than decorative.
- Use restrained forward geometry—directional lines, signal nodes, timing ticks and a subtle 10–14 degree lean—to suggest speed. Do not use racing stripes, motion blur or an aggressive italic wordmark that harms legibility.
...
- Use *Signals into execution* as the short brand line and *Automated funding-rate arbitrage* as the plain-language descriptor. The compact app header and icon use the name or mark alone.
...
- Complete trademark, domain, app-store and social-handle clearance for **Signex**; finalize the signal-and-velocity brand assets, ownership, legal/risk disclosures and design identity.
...
- **Signex** remains a working public name until trademark, domain, app-store and social-handle clearance is complete; venue eligibility and regulatory review also remain pre-launch business decisions.
